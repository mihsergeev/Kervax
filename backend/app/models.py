...
class Server(Base):
    ...
    # Доля ответов 5xx за 15 минут, % (0=выкл). По умолчанию 0.05 - так откалибровали
    # алерт по балансерам в прометеусе: там сломанный DNS давал 0.1-0.28%, а до поломки
    # ошибок почти не было. Мелкие ноды страхует нижний порог по числу ошибок.
    web_5xx_alert_percent: Mapped[float] = mapped_column(Float, default=0.05)
