...
def check_agent_release_matches() -> None:
    """Версия агента в исходниках должна совпадать с подписанным манифестом.

    Панель собирает бинарь агента сама, а manifest.json кладёт релизный скрипт.
    Поднял версию в agent/main.go и забыл переподписать — панель начинает раздавать
    бинарь, чей sha не совпадает с подписанным: в «Требует действий» повисает
    «релиз агента собран неправильно», а самообновление молча не едет у всего парка.
    Ловится ровно здесь, до релиза. Манифест не в репозитории (.gitignore), поэтому
    у кого его нет — проверка просто пропускается.
    """
    path = "agent-dist/manifest.json"
    if not os.path.exists(path):
        print("подписанного релиза агента нет — сверять не с чем")
        return
    src = re.search(r'^const version = "([\d.]+)"', read("agent/main.go"), re.M)
    try:
        signed = json.loads(read(path)).get("version")
    except ValueError:
        fail("agent", f"{path} не читается как JSON")
        return
    if not src:
        fail("agent", "не нашёл const version в agent/main.go")
    elif src.group(1) != signed:
        fail("agent", f"агент в исходниках {src.group(1)}, а подписан релиз {signed} — "
                      f"панель будет раздавать неподписанный бинарь; "
                      f"перевыпустите: python agent-signing/release.py {src.group(1)}")
    else:
...
def check_compose_file_pinned() -> None:
    ...
    Без этой строки «docker compose up -d» в каталоге панели видит один базовый
    compose.yml: тянет не готовые образы, а сборку из исходников, и пересоздаёт
    контейнеры БЕЗ сети caddy и его меток. Панель молча пропадает со своего
    домена — а это ровно то, что делает документированная команда обновления,
    у которой нет флагов -f.
    """
    qs = read("ops/quickstart.sh")
    if "COMPOSE_FILE=" not in qs:
        fail("compose", "quickstart.sh не записывает COMPOSE_FILE в .env — "
                        "обновление пересоберёт панель из исходников и снимет её с домена")
    if not re.search(r"^COMPOSE_FILE=compose\.yml:", read(".env.example"), re.M):
        fail("compose", ".env.example без действующей строки COMPOSE_FILE: "
                        "ручная установка останется без набора оверлеев")
    for doc in ("docs/install.ru.md", "docs/install.md"):
        if "COMPOSE_FILE" not in read(doc):
            fail("compose", f"{doc} не упоминает COMPOSE_FILE, а команда обновления на него опирается")

    # Печатная команда, где есть scale, но нет caddy, снимает панель с домена:
    # набор -f не дополняет прежний запуск, а задаёт его целиком. Проверено
    # вживую — контейнеры Up, scheduler работает, домен молчит.
    for doc in ("docs/install.ru.md", "docs/install.md", "README.ru.md", "README.md"):
        for line in read(doc).split("\n"):
            cmd = line.strip()
            if not cmd.startswith(("docker compose", "sudo docker compose")):
                continue
            if "compose.scale.yml" in cmd and "compose.caddy.yml" not in cmd:
                fail("compose", f"{doc}: команда со scale без compose.caddy.yml — "
                                f"выполнив её, читатель снимет панель с домена: {cmd[:70]}…")
    print("COMPOSE_FILE закреплён: quickstart, .env.example и обе инструкции; "
          "scale-команды в документации идут вместе с caddy")
