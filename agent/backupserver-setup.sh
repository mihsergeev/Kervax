...
KERVAX_SETUP_VERSION=0.19  # МАЖОР.МИНОР; сравнивается покомпонентно (0.13 > 0.2!)
...
cmd_stats() {
  ...
  printf '{"present":true,"version":"%s","helper_version":%s,"running":%s,"tls_front":%s,"tls_port":%s,"data_dir":"%s","disk_total":%s,"disk_used":%s,"disk_free":%s,"repos":[%s]}\n' \
    "$ver" "$HELPER_VER" "$running" "$tls" "$TLS_PORT" "$(json_escape "$DATA")" "$d_total" "$d_used" "$d_free" "$repos_json"
}
...
cmd_update_image() {
  ...
  local port cur
  port="$(grep -oE '"[0-9]+:8000"' "$COMPOSE" | head -1 | grep -oE '^"[0-9]+' | tr -d '"')"
  [ -n "$port" ] || port="$REST_PORT"
