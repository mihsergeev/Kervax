...
// Причина последнего отказа самообновления — уходит в отчёт, чтобы её было видно
// в панели, а не только в journalctl конкретной ноды. Самая частая причина —
...
func lastUpdateErrorText() string {
	v, _ := lastUpdateError.Load().(string)
	if len(v) > 300 {
		v = v[:300]
	}
	return v
...
type report struct {
	...
	// Почему не поехало самообновление. Агент отказывает безопасно — но раньше
	// молча: причина оставалась в journalctl ноды, а панель показывала лишь
	// «цель 2.0 / версия 1.99» без объяснений. Самая частая причина — панель
	// собрана НЕ с тем пубключом, которым подписан релиз; на парке в десятки
	// нод искать это по логам каждой ноды невозможно.
...
type backupServerInfo struct {
	...
	// HTTP-порт rest-server: панель разворачивает его на выбранном порту и по нему
	// же должна создавать репозиторий клиента. Без этого поля она подставляла
	// константу 64100 — сервер на другом порту разворачивался, а настройка
	// клиента падала на init репозитория.
...
func collect(prev sample) (report, sample) {
	...
	r := report{
		...
		UpdateError:   lastUpdateErrorText(),
		Docker:        dk,
