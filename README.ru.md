...
Мониторинг корпоративного уровня, готовый к работе сразу после установки: сайты,
серверы, контейнеры, базы данных и бэкапы — в одной панели. Установка выполняется
одной командой, агент сам определяет, что работает на машине, дашборды заполняются
с первой проверки, алерты уходят в Telegram или webhook.

Инфраструктура видна с двух сторон одновременно. Снаружи — внешними проверками:
код ответа, ключевое слово, время отклика, сроки сертификата и домена, в том числе
через прокси в других сетях. Изнутри — агентом, который работает **на исходящих**
соединениях: на ноде не открывается ни один порт, а вместо токена панель хранит
его хеш.
...
Команда установит Docker (если его нет), поднимет Caddy с сертификатом
Let's Encrypt и выведет адрес панели с паролем администратора. Свой домен не
обязателен: без него панель работает на `<IP-сервера>.sslip.io`.

![Главная](docs/img/ru/home.png)

<table>
<tr>
<td width="50%"><a href="docs/img/ru/servers.png"><img src="docs/img/ru/servers.png" alt="Серверы"></a><br><sub>Серверы: список парка с текущей нагрузкой</sub></td>
<td width="50%"><a href="docs/img/ru/server-detail.png"><img src="docs/img/ru/server-detail.png" alt="Деталь сервера"></a><br><sub>Карточка сервера: CPU, память, диски, сеть, процессы</sub></td>
</tr>
<tr>
<td width="50%"><a href="docs/img/ru/sites.png"><img src="docs/img/ru/sites.png" alt="Сайты"></a><br><sub>Сайты: доступность, сертификаты, домены, проверка из нескольких точек</sub></td>
<td width="50%"><a href="docs/img/ru/site-detail.png"><img src="docs/img/ru/site-detail.png" alt="Деталь монитора"></a><br><sub>Карточка монитора: доступность, время отклика по точкам, инциденты</sub></td>
</tr>
</table>

<sub>Скриншоты сняты с работающей панели на демонстрационных данных; это тот же
фронтенд, что попадает в образ. Все хосты, домены и адреса в них вымышлены
...
**Что работает на серверах.** Контейнеры Docker и поды Kubernetes,
...
**Что истечёт раньше, чем сломается.** Кластеры умирают по датам: истёк токен
Flux — и в кластер больше ничего не приезжает, притом что запущенное продолжает
работать, а дашборды остаются зелёными. Kervax читает сроки сертификатов
control-plane и kubelet, kubeconfig-ов, TLS-секретов и кредов, которыми Flux
ходит в Git, и предупреждает за две недели. Плюс следит за `Ready` у ресурсов
Flux — на случай, когда токен не истёк, а отозван. Даты считает root-хелпер на
самой ноде: ни ключей, ни значений токенов панель не видит.

**Сайты, закрытые снаружи.** Внутренняя панель, графана, n8n за белым списком IP:
панель до них не дотянется, а монитор будет вечно «недоступен». Такой сайт
проверяет агент изнутри сервера — стучится на `localhost` с нужным именем хоста и
шлёт результат. Соединяется он ТОЛЬКО с localhost, так что панель не может послать
его никуда ещё; наружу уходят код ответа и задержка, а не страница. В списке такой
монитор помечен «локально».
